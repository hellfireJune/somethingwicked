--[[SomethingWicked.ItemRenderHelper = {}
...
local function AddEggPlayers(i, player)
	--[[local curCharges = SomethingWicked.ItemHelpers:CheckPlayerForActiveData(player, EEVEEMOD.CollectibleType.STRANGE_EGG)
...
function SomethingWicked.ItemRenderHelper:UpdatePlayers()
...
local function GetBookFrame(player)
	...
	local hasBelial = player:HasCollectible(CollectibleType.COLLECTIBLE_BOOK_OF_BELIAL_PASSIVE)
	local verdict = (hasVirtues and hasBelial) and 2 or (hasVirtues or hasBelial) and 1 or 0

	return verdict
end

function SomethingWicked.ItemRenderHelper:ShowItemOutline()
...
function SomethingWicked.ItemRenderHelper:OnRender()
	SomethingWicked.ItemRenderHelper:UpdatePlayers()
	SomethingWicked.ItemRenderHelper:ShowItemOutline()
end

function SomethingWicked.ItemRenderHelper:ResetOnGameStart()
	for i = 1, 4 do
		EggPlayers[i].Player = nil
	end
end]]