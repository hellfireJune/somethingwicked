...
function MenuProvider.GetPaletteSetting()
    return mod.save.MenuPalette
end

function MenuProvider.SavePaletteSetting(var)
    mod.save.MenuPalette = var
end

function MenuProvider.GetHudOffsetSetting()
    if not REPENTANCE then
        return mod.save.HudOffset
    else
...
function MenuProvider.SaveHudOffsetSetting(var)
    if not REPENTANCE then
        mod.save.HudOffset = var
    end
end

function MenuProvider.GetGamepadToggleSetting()
    return mod.save.GamepadToggle
end

function MenuProvider.SaveGamepadToggleSetting(var)
    mod.save.GamepadToggle = var
end

function MenuProvider.GetMenuKeybindSetting()
    return mod.save.MenuKeybind
end

function MenuProvider.SaveMenuKeybindSetting(var)
    mod.save.MenuKeybind = var
end

function MenuProvider.GetMenuHintSetting()
    return mod.save.MenuHint
end

function MenuProvider.SaveMenuHintSetting(var)
    mod.save.MenuHint = var
end

function MenuProvider.GetMenuBuzzerSetting()
    return mod.save.MenuBuzzer
end

function MenuProvider.SaveMenuBuzzerSetting(var)
    mod.save.MenuBuzzer = var
end

function MenuProvider.GetMenusNotified()
    return mod.save.MenusNotified
end

function MenuProvider.SaveMenusNotified(var)
    mod.save.MenusNotified = var
end

function MenuProvider.GetMenusPoppedUp()
    return mod.save.MenusPoppedUp
end
    ...
    settings = {
        ...
        buttons = {
...

            {
                ...
                load = function()
                    return mod.save.exampleoption or 1
                end,
            ...
            {
                str = 'slider option',
                -- The "slider" tag allows you to make a button a slider, with notches that are
                -- transparent / opaque depending on if they are filled.
                slider = true,
                -- Increment determines how much the value of the slider changes with each notch
                increment = 1,
                -- Max determines the maximum value of the slider. The number of notches is equal to
                -- max / increment!
                max = 10,
                ...
                load = function()
                    return mod.save.exampleslider or 1
                end,
                store = function(var)
                    mod.save.exampleslider = var
                end,
            ...
            },
            {
                ...
                load = function()
                    return mod.save.examplenumber or 20
                end,
                store = function(var)
                    mod.save.examplenumber = var
                end,
            ...
            },
            {
                ...
                load = function()
                    return mod.save.examplekeybind or -1
                end,
                store = function(var)
                    mod.save.examplekeybind = var
                end,
            ...
            },
            {
                ...
                -- The "generate" function is run the very first time a button displays upon
                -- switching to its item. You can use this to change the button's data dynamically,
                -- for instance for a menu that uses non-constant data. In this example, it's just a
                -- random chance to change the string the button displays, but you could do pretty
                -- much anything!
...
DeadSeaScrollsMenu.AddMenu("Something Wicked", {
